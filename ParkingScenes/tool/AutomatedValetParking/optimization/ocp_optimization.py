...
solver_path=os.path.join(current_dir, 'ipopt')
...
class ocp_optimization:
    ...
    def solution(self, path: list):
        ...
        for i in range(len(initial_solution)):
            # theta — normalize to [-π, π] to match Pyomo variable bounds
            if (i-2) % 7 == 0:
                while initial_solution[i] > math.pi:
                    initial_solution[i] -= 2 * math.pi
                while initial_solution[i] < -math.pi:
                    initial_solution[i] += 2 * math.pi
            # velocity
            elif (i-3) % 7 == 0:
        ...
        optimal_tf = pyo.value(model.variables[variable_n-1]) or 0.0
        optimal_dt = optimal_tf / (points_n-1) if points_n > 1 else 0.0
        optimal_traj = []
        points = []
