...
def plot_episode(ep_dir: pathlib.Path, cache_path=None, frame_idx=None):
    ...
    ax_world.set_title('World trajectory', fontsize=9)
...
def main():
    ...
    if args.episode:
        ep_dir = _RAW / args.episode
    else:
        episodes = sorted(_RAW.glob('episode_*'))
        if not episodes:
            print(f"No episodes found in {_RAW}")
            raise SystemExit(1)
        ep_dir = episodes[-1]
